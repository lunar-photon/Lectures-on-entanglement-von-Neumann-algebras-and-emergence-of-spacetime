\section{Sec.~X: Conclusions and discussions}

\subsection{Sec.~X.A: summary}
...
\item Large-$N$ boundary algebras split cleanly into two kinds, worth keeping distinct in your own head going
forward: an \textbf{entanglement wedge algebra} $X$ is, by definition, the large-$N$ limit of a genuine
finite-$N$ algebra $B$ — it always admits a finite-$N$ ancestor. A \textbf{causal wedge algebra} $Y$, by
...
\item Simple operator-algebraic toy models — static observers (Sec.~IX.A--D), dynamical observers
...
\subsection{Sec.~X.B: the mathematical structure of quantum gravity, and the full-circle answer to where we
started}
...
with in Sec.~I: \emph{what happens to the wavefunction, and to the Hilbert space itself, once you take the
algebra seriously as the fundamental object?}

\subsubsection*{Why quantum gravity cannot have one single Hilbert space}
...
change a spacetime's \emph{asymptotic} structure — there's no physical process that takes a state of type IIB
...
\textbf{each of these asymptotic backgrounds must correspond to a genuinely separate Hilbert space} — and
...
supposed to be different vacua of the very same underlying theory — type IIB string theory. \textbf{Without
one single, global Hilbert space to hold all of these together, what mathematical object is left to call ``the
theory''?}

\subsubsection*{The resolution you've already met, twice}
...
space can be quantized using ordinary Minkowski time, \emph{or} using Rindler time (Sec.~IV.D.1) — both are
perfectly physically sensible choices, describing different physical situations (an inertial vs.\ an
accelerated observer), and yet they produce genuinely, unitarily \emph{inequivalent} Hilbert spaces. You could
...
\textbf{Algebraic QFT resolves this by defining the theory as a pair $(\Alg,\Sscr)$ — an algebra $\Alg$ of
...
\emph{derived}, state-dependent object, produced fresh by the GNS construction of Sec.~II.D for whichever
state $\omega\in\Sscr$ you happen to pick.} Minkowski quantization and Rindler quantization are simply two
...
\textbf{The proposal here is to do exactly this, one level up, for the whole of quantum gravity.} Postulate
...
correspond to \emph{different states} $\omega$ on this one algebra, each producing its own GNS Hilbert space
$\HH_\omega$ via exactly the Sec.~II.D construction, with the corresponding boundary CFT (where one is known)
simply \emph{being} that GNS representation. Concretely: $\omega_{\text{AdS}_5\times S^5}$ is the state whose
...
same idea:
\begin{itemize}
\item \textbf{Sec.~II.B.3 / Slavnov}: an ordinary quantum \emph{state} $\omega$ (or $\rho$, or $\ket\psi$) is
...
\item \textbf{Sec.~II.D / the Minkowski--Rindler example just above}: the \emph{Hilbert space itself} is not
fundamental — a single algebra $\Alg$ can produce many inequivalent Hilbert spaces, one per choice of
reference state, via GNS.
\item \textbf{Here}: even the question ``which spacetime background am I in'' is not fundamental data fed
...
that — causal structure, horizons, entropy) emerging as a \emph{consequence} of that choice, not an
ingredient of it.
\end{itemize}
...
are not fixed ingredients handed to the theory in advance, but \emph{derived, state-dependent consequences} of
...
content beyond what this companion has already covered in the main text: \textbf{Appendix A} works through
...
of this companion, that infinite entanglement generically prevents factorization); \textbf{Appendix B} carries
