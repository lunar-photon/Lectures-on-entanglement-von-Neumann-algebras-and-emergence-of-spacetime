...
\section{What problem are these notes about?}
...
happens \emph{on} that stage. General relativity complicates this a little, because the geometry itself is
...
well-defined notion of ``here,'' a causal order, a notion of a local region. \textbf{What, physically and
...
theory of quantum gravity in a $(d{+}1)$-dimensional \emph{anti-de Sitter} spacetime is conjectured to be
...
chosen because it has a well-defined boundary at infinity. The boundary theory is a \emph{conformal field
theory} (CFT).
...
question above then becomes concrete: \emph{how does bulk spacetime, with its geometry, causal structure, and
locality, emerge from an ordinary quantum field theory on a lower-dimensional boundary, as the number of
degrees of freedom of that theory is sent to infinity?}

The Ryu--Takayanagi proposal~\cite{RT} gave the first strong hint of the answer. It is a formula that relates
...
developed by Leutheusser, Liu, and collaborators~\cite{Liu2025}. The key tool is a piece of mathematics from
...
\section{From wavefunctions to density matrices to algebraic states}
...
assumes a \emph{closed, isolated system}, in which an experimenter can in principle measure any operator on
the whole system.

\item \textbf{Level 1: the density matrix $\rho(x, x')$ and the Wigner function.}
...
gives the \textbf{Wigner quasi-probability distribution} on phase space (we set $\hbar=1$):
\[
...
\item \textbf{Level 2: observers have limited access.}
...
$D(\rho\|\sigma) = \Tr(\rho\log\rho - \rho\log\sigma)$ between two \emph{different} global pure states is
...
relative entropy of the two states \emph{restricted to} $\M$, which is typically finite and informative. In
...
a fixed global Hilbert space is fundamental. The primary physical object is the \textbf{algebra of accessible
observables}, $\M$. A \textbf{state} $\omega$ is simply a positive linear functional that assigns
...
They are \textbf{constructed} from the state $\omega$ by the Gelfand--Naimark--Segal (GNS) construction of
...
\begin{keyresult}
\textbf{The conceptual takeaway:} The wavefunction $\psi(x)$ is not the fundamental container of physical
...
\section{Why the ordinary quantum-mechanical definition of ``subsystem'' isn't good enough}
...
\textbf{entangled} exactly when the array $c_{ia}$ cannot be written as a product $c_{ia}=\psi_i\chi_a$ of
...
\section{The central example: infinitely many entangled qubit pairs}
...
\end{align}
\stepwhy{1}{This is the definition of the partial trace: sandwich between the basis vectors of $L$ and sum.}\quad
\stepwhy{2}{${}_L\braket{0|\phi_\theta}=\cos\theta\ket0_R$ and ${}_L\braket{1|\phi_\theta}=\sin\theta\ket1_R$.
The cross terms $\ket{00}\bra{11}$ and $\ket{11}\bra{00}$ drop out because $\braket{0|1}_L=0$.}\quad
\stepwhy{3}{Write the result as a matrix in the basis $\ket0_R,\ket1_R$.}
...
\begin{itemize}
\item For the maximally entangled Bell pair at $\theta=\pi/4$, we have $\cos^2(\pi/4)=\sin^2(\pi/4)=1/2$,
so
\[
s_1(\pi/4) = -\left[ \tfrac12\log(\tfrac12) + \tfrac12\log(\tfrac12) \right]
= \log 2 \approx 0.693147\text{ nats} \quad (= 1\text{ bit}).
\]
\item In the weakly entangled limit $\theta \to 0$:
\begin{align}
...
\end{align}
\stepwhy{1}{Substitute $\cos^2\theta=1-\theta^2+O(\theta^4)$ and $\sin^2\theta=\theta^2+O(\theta^4)$.
The neglected $O(\theta^4)$ corrections change the result only at order $\theta^4\log\theta$.}\quad
\stepwhy{2}{Expand $\log(1-\theta^2)=-\theta^2+O(\theta^4)$ in the first term, and use
...
\subsection*{Restricting to finite energy}
...
A \textbf{finite-energy state} built on the reference state $\ket{\Phi_\theta}$ is one obtained by acting on
...
\end{align}
\stepwhy{1}{Here $\rho_i$ is the reduced density matrix of pair $i$ in the state
$\ket\psi_R\ket\chi_L$. The expectation value of $P_i$ depends only on $\rho_i$.}\quad
\stepwhy{2}{Because the global state is a product across $R|L$, $\rho_i=\rho_{R_i}\otimes\rho_{L_i}$, which is
...
\emph{unentangling the $R$ and $L$ subsystems would require changing infinitely many pairs, and so would cost
infinite energy.} ``Unentangling'' has a concrete operational meaning here. It means applying some joint
...
\begin{enumerate}
\item \textbf{Every finite-energy state has infinite entanglement.} Take any state obtained from
...
\item \textbf{The Hilbert space itself does not factorize.} Suppose a tensor factorization
...
$N$-pair system as $N\to\infty$, before restricting to finite energy. It is not even a \emph{separable}
...
\section{Two more examples, more briefly}
...
\textbf{Lattice gauge theory.} A gauge theory (such as electromagnetism, or the theory of the strong force)
...
are related by \emph{gauge transformations}, $U_{ij}\to V_iU_{ij}V_j^\dagger$. Physical states are required to
...
\textbf{A quantum field theory cut in half.} Take a relativistic quantum field theory, say a scalar field, and
...
the cut in \emph{every} finite-energy state, including the vacuum, the lowest-energy state of all.
...
diverges as the cutoff is removed. It diverges in a specific way: in proportion to the \emph{area} of the
...
\section{Explicit derivation: why the continuum QFT Hilbert space cannot factorize}
...
\subsection*{1. The Hamiltonian and the gradient coupling across the cut}
...
$\partial R$. This surface is called the \emph{entangling surface}.
...
\subsection*{2. Two oscillators across the cut}
...
\subsection*{3. The vacuum: neighbours across the cut move together}
...
\begin{itemize}
\item The difference between the two sides is small:
\begin{align}
...
\end{align}
\stepwhy{1}{$\phi_R-\phi_L=\sqrt2\,\phi_-$, from the normal-mode definition.}\quad
\stepwhy{2}{A harmonic-oscillator ground state of frequency $\omega$ has $\braket{\phi^2}=1/(2\omega)$.}\quad
\stepwhy{3}{Expand $1/\omega_-=(\epsilon/\sqrt2)(1+M^2\epsilon^2/2)^{-1/2}$ for small $\epsilon$.}
\item The energy stored in the link across the cut is therefore
\begin{equation}
...
\subsection*{4. The energy cost of an unentangled state}
...
\end{align}
\stepwhy{1}{Expand the square and factorize the cross term using \eqref{eq:product_state_covariance_zero}.}\quad
\stepwhy{2}{Write $\braket{\phi^2}=(\Delta\phi)^2+\braket{\phi}^2$, where $(\Delta\phi)^2$ is the variance,
...
\end{align}
\stepwhy{1}{Expand $(\phi_R-\phi_L)^2$ in \eqref{eq:pair_hamiltonian}. The $\phi_R^2$ and $\phi_L^2$ pieces
raise $M^2$ to $\Omega_\epsilon^2$. The cross term factorizes by \eqref{eq:product_state_covariance_zero}.}\quad
\stepwhy{2}{Shift $\phi_R$ and $\pi_R$ by their mean values. Then $\braket{h_R}$ splits into
...
\stepwhy{3}{Use $ab\le\tfrac12(a^2+b^2)$.}\quad
\stepwhy{4}{$\Omega_\epsilon^2-1/\epsilon^2=M^2$.}
...
\textbf{Check on the full chain.} The two-site truncation is crude, so we checked the result numerically on
...
\subsection*{5. Summing over the entangling surface}
...
\end{align}
\stepwhy{1}{The transverse modes are independent, so their minimum excitation energies add.}\quad
\stepwhy{2}{Each mode costs at least $c_{\min}/\epsilon$, by \eqref{eq:delta_e_single_mode}.}\quad
\stepwhy{3}{The integral is the volume of a ball of radius $1/\epsilon$ in $d-2$ dimensions, divided by
...
\begin{enumerate}
\item \textbf{An infinite energy barrier.} The energy needed to disentangle a region $R$ from its
...
\item \textbf{No product state has finite energy.} The physical Hilbert space of a continuum field theory, the
...
\item \textbf{Failure of factorization.} A tensor product $\HH_L \otimes \HH_R$ is, by definition, spanned by
...
\subsection*{6. The algebraic version: type $\mathrm{III}_1$ and the split property}

The energy divergence has a precise algebraic counterpart, which we quote without proof.
\begin{itemize}
\item Suppose two spatial regions $R_1$ and $R_2$ are separated by a \textbf{finite buffer} of width
...
$R_2$ \emph{can} be prepared. Heuristically, the estimate above with $\epsilon$ replaced by $\delta$ suggests
that its energy cost is finite but grows without bound as $\delta\to0$. The algebraic statement is the
\textbf{split property} of quantum field theory (established under mild assumptions by Buchholz,
...
\item As the buffer shrinks to zero ($\delta \to 0$, so that the regions touch along a common boundary), this
intermediate type I factor ceases to exist, and the estimated energy cost diverges. What remains is the
algebra $\M(R)$ of the region itself, which is not type I or type II. It is a \textbf{type $\mathrm{III}_1$
factor}, the most strongly entangled type in the classification (Chapter~4).
\end{itemize}

\section{The fix: define a subsystem by what you can \emph{do}, not by how you'd \emph{split the Hilbert
space}}

The way out is simple to state before any formalism is attached to it. Do not define a subsystem $R$ by
declaring a factorization $\HH=\HH_R\otimes\HH_L$ in advance. Instead, define it operationally: \emph{$R$ is
whatever an observer confined to that subsystem can measure or do.} Concretely, that observer has access to
...
with these properties is a \textbf{von Neumann algebra}. There are two main candidate notions of ``limit,''
...
2021 by Leutheusser, Liu, Witten, and others~\cite{Liu2025}. It is that this same classification, applied to
...
At this point these claims are \emph{stated}, not yet demonstrated. The double-headed arrow above is not a
...
Similarly, this chapter only motivates the deeper claim that entanglement structure is what \emph{builds}
bulk spacetime geometry. The motivation comes from the Ryu--Takayanagi formula and from the broader
``entanglement builds geometry'' literature (Van~Raamsdonk~\cite{VanRaamsdonk},
Maldacena--Susskind~\cite{MaldacenaSusskind}). The actual mechanism is subregion-subalgebra duality: the
...
\section{Plan of the notes}

The notes split into two halves. Where the split falls says something about how the argument is built.

\textbf{Chapters~2--5: the mathematics, with no gravity or holography.} This half is pure operator-algebra
...
about type III needs a new tool, Tomita--Takesaki modular theory. Chapter~5 introduces the \emph{crossed
product}. This construction takes a type III algebra, attaches an auxiliary quantum clock to it, and produces
...
\textbf{Chapters~6--10: applying the machinery to quantum gravity.} Chapter~6 sets up the large-$N$
...
\begin{itemize}
\item \textbf{Large $N$ versus finite $N$.} ``Large $N$'' (equivalently $G_N\to0$) means working in a
...
\emph{bounded} operators on $\HH$. An operator $A$ is bounded if there is some finite number $c$ such that
...
Some familiar operators are \emph{not} bounded. Position $\hat x$ and momentum $\hat p$ for a particle on a
...
\item \textbf{$\widetilde J^\pm(Y)$: causal future and past.} For a region $Y$ of a spacetime,
...
The tilde means that these sets are computed in the \emph{conformal completion} of the spacetime. This is a
...
\item \textbf{Causal complement (single prime) and causal completion (double prime) of a spacetime region.}
...
The causal completion $Y''$ is the double complement, $(Y')'$. In general $Y''$ is \emph{larger} than $Y$.
...
\emph{domain of dependence} (defined next). This is a bigger set, which extends into the past and the future.
...
\item \textbf{$\bar R$, complement of a spatial region; $\hat R$, domain of dependence.} Let $R$ be a
...
$\hat R$ is a spacetime region built from $R$. It is the set of spacetime points $p$ such that \emph{every}
...
\emph{dependence}. If $R$ is a full spatial slice, $\hat R$ is the entire spacetime. If $R$ is a proper
subregion, $\hat R$ is a diamond-shaped region, called a \emph{causal diamond}. It sits above and below $R$
and is bounded by the light rays that just graze the edge of $R$. This shape appears constantly from
Chapter~6 onward.

\item \textbf{$\subset$ always means \emph{proper} subset.} Whenever you see $\M_1\subset\M_2$ in these notes,
