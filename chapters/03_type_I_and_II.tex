\chapter{Von Neumann algebras and entanglement: type I and II}
...
answer. Given $\Psi$, what does the \emph{type} of $\M$ tell us about how $\Psi$ entangles the subsystem
...
\toprule
\textbf{Factor} & \textbf{Values of $d(P)$} & \textbf{Trace} & \textbf{Density operator} &
\textbf{Example} & \textbf{Entropy} \\
\midrule
...
\section{Density operators for type I and II algebras}
...
solve eq.~\eqref{eq:rhoM-def} \emph{inside} the algebra. (If $\rho_\M$ is unbounded, the precise statement is
that $\rho_\M$ is \emph{affiliated} with $\M$: all of its spectral projections lie in $\M$.)
...
words, to build an object that describes the part of the world you \emph{can} access, you must know about the
part you \emph{cannot} access. Equation~\eqref{eq:rhoM-def} needs nothing of the sort. It uses only the trace of
...
entanglement with the outside world? The answer depends on the \emph{type} of $\M$. Working it out type by type
is the content of the rest of this chapter and of Chapter~4.

\section{Type I algebras}

\subsection*{The type I factor case: nothing is lost}

When $\M$ is a type I \emph{factor}, Chapter~2 showed that
\[
...
\end{align}
\stepwhy{1}{an expectation value is the trace of the operator times the projector $\ket\Psi\bra\Psi$.}\quad
\stepwhy{2}{do the trace over $\HH_L$ first; the operator $a$ does not act on $\HH_L$, so it comes out of that
partial trace.}\quad
\stepwhy{3}{definition of $\rho_R$.}\quad
\stepwhy{4}{$(a\otimes\id_L)(\rho_R\otimes\id_L)=a\rho_R\otimes\id_L$, and $\tr$ of this is $\Tr_{\HH_R}(a\rho_R)$.}
...
and the old one, which uses a partial trace, give \emph{exactly} the same operator whenever a type I
...
\begin{enumerate}
\item The two recipes are conceptually different, even though they agree here. The partial-trace recipe needs
...
\item Here $\tr$ equals the ordinary $\Tr_{\HH_R}$, which counts basis states of an honest Hilbert space
...
\subsection*{General type I: a nontrivial center, and where lattice gauge theory fits}

Now suppose $\M$ is type I but \emph{not} a factor. Then it has a nontrivial center. (Recall the
...
\end{align}
\stepwhy{1}{$A$ is block diagonal, so only the diagonal blocks $p_\alpha\rho_\alpha$ of $\rho$
contribute.}\quad
\stepwhy{2}{the type I factor computation above, done inside each sector.}\quad
\stepwhy{3}{the sector-by-sector formula for $\tr$.}
...
\label{eq:SM-center}
\end{align}
\stepwhy{1}{$\rho_\M$ is block diagonal, so $\log\rho_\M$ is computed block by block, and $\tr$ is the sum of
the traces of the $R$ parts.}\quad
\stepwhy{2}{$\log(cX)=\log c\,\id+\log X$ for a number $c>0$.}\quad
\stepwhy{3}{$\Tr_{\HH_{R\alpha}}\rho_{R\alpha}=1$, and the definition of $S_\alpha$.}

The entropy splits into two recognizable pieces. The first is the classical (Shannon) entropy of \emph{which
sector you are in}. The second is the probability-weighted average of the quantum entanglement entropy
\emph{within} each sector. Nothing here is conceptually new. It is what you would compute by hand if someone
...
\subsection*{Building a trace at $\theta=\pi/4$}
...
$\theta=\pi/4$, and only there, this expectation value \emph{also} has the cyclic property $\tr(AB)=\tr(BA)$
that defines a trace (Chapter~2). The rest of this section rests on this single fact, so we verify it.

\textbf{Step 1: a single pair.} Take one spin pair at $\theta=\pi/4$, in the state
...
\end{align}
\stepwhy{1}{substitute $\ket{\phi_{\pi/4}}=\frac1{\sqrt2}(\ket{00}+\ket{11})$ on both sides.}\quad
\stepwhy{2}{expand the product of two two-term sums into four terms.}\quad
\stepwhy{3}{$a\otimes\id_L$ acts only on the first (right) qubit, so each term factorizes into a matrix
element of $a$ times an inner product on $L$.}\quad
\stepwhy{4}{$\braket{0|0}_L=\braket{1|1}_L=1$ and $\braket{0|1}_L=\braket{1|0}_L=0$, so the cross terms
vanish.}\quad
\stepwhy{5}{$a_{00}+a_{11}$ is the ordinary matrix trace $\Tr_2(a)$.}
...
\textbf{Step 2: many pairs.} A typical element of $\Alg_R$ is a product $A=a_1\otimes a_2\otimes\cdots$, where
...
\end{align}
\stepwhy{1}{both $A$ and $\ket{\Phi_{\pi/4}}$ are products over pairs, so the expectation value is a product of
single-pair expectation values.}\quad
\stepwhy{2}{each touched pair gives $\tfrac12\Tr_2(a_{i_s})$ by eq.~\eqref{eq:typeII-single-pair}; each
...
\textbf{Step 3: cyclicity.} Take two product operators $A=\bigotimes_i a_i$ and $B=\bigotimes_i b_i$. Choose $n$
...
\ \eqstep{3}\ \tr(BA) . \notag
\end{align}
\stepwhy{1}{eq.~\eqref{eq:typeII-many-pairs} applied to the product operator $AB$; pairs where $a_ib_i=\id_2$
contribute $1$.}\quad
\stepwhy{2}{the ordinary matrix trace is cyclic, $\Tr_2(a_ib_i)=\Tr_2(b_ia_i)$, pair by pair.}\quad
\stepwhy{3}{step (1) read backwards, for the product operator $BA$.}
...
More is true: for $\theta\neq\pi/4$ the algebra $\M_R$ admits no normalized trace of \emph{any} kind. This can
...
\textbf{Theorem.} Let $\Alg_R = \bigotimes_{k=1}^\infty M_2(\mathbb{C})$ be the algebra of the right-hand spins,
...
algebra given by the GNS construction. If $\theta \ne \pi/4$, then \textbf{there is no normal tracial state on
$\M_R$}.

\textbf{Proof.}
\begin{enumerate}[leftmargin=*]
\item \textbf{A trace vanishes on each $\sigma_z$.}
...
\end{align}
\stepwhy{1}{substitute the commutator identity and pull the constant $\tfrac1{2i}$ out by linearity of
$\tau$.}\quad
\stepwhy{2}{a trace vanishes on every commutator.}

\item \textbf{A trace vanishes on the average magnetization.}
...
\end{align}
\stepwhy{1}{linearity of $\tau$.}\quad
\stepwhy{2}{every term vanishes by item~1.}

\item \textbf{In the GNS state, the magnetization settles to a fixed value.}
...
\eqstep{3} \cos(2\theta) . \notag
\end{align}
\stepwhy{1}{$\sigma_z$ is diagonal, so the cross terms between $\ket{00}$ and $\ket{11}$ vanish. What remains
are the two diagonal terms, weighted by $\cos^2\theta$ and $\sin^2\theta$.}\quad
\stepwhy{2}{$\braket{0|\sigma_z|0}=+1$ and $\braket{1|\sigma_z|1}=-1$.}\quad
\stepwhy{3}{the double-angle identity.}
...
\end{align}
\stepwhy{1}{write $M_N-c\,\id=\frac1N\sum_k(\sigma_z^{(k)}-c)$, expand the squared norm into a double sum, and
use the correlation formula just derived.}\quad
\stepwhy{2}{the Kronecker delta leaves $N$ equal terms.}
...
\item \textbf{From one vector to all vectors.}
We now show that $M_N\to c\,\id$ on \emph{every} vector, not only on $\ket{\Omega_\theta}$. Take any $A'$ in
...
\end{align}
\stepwhy{1}{$A'$ commutes with $M_N$.}\quad
\stepwhy{2}{the operator-norm bound $\|A'v\|\le\|A'\|\,\|v\|$.}
...
\ \eqstep{4}\ \cos(2\theta) . \notag
\end{align}
\stepwhy{1}{normality of $\tau$, applied to the bounded sequence $M_N$ and its strong limit from
item~4.}\quad
\stepwhy{2}{substitute the limit $\mathrm{s\text{-}}\lim M_N=\cos(2\theta)\id$.}\quad
\stepwhy{3}{linearity of $\tau$.}\quad
\stepwhy{4}{normalization, $\tau(\id)=1$.}
...
\emph{normalized} trace, that is, types $\mathrm{II}_1$ and $\mathrm{I}_n$ with finite $n$. It does not by
...
\subsection*{No minimal projection, and dimensions that are real numbers}
...
qubit. In general, we can choose \emph{any} finite set of spins and replace their identity factors with
$P_\uparrow$.

\textbf{Dimensions.} We use the trace normalized so that $d(\id)=\tr(\id)=1$. This holds automatically, since
...
\textbf{No minimal projection.} Take any nonzero projection $P$ of this fixed-spin form. It fixes only finitely
...
\textbf{Every dimension in $[0,1]$.} Here something appears that has no counterpart in finite-dimensional
...
\subsection*{Interpreting $\rho_\M$ and $S_\M$: an entropy that can be negative}
...
\end{align}
\stepwhy{1}{eq.~\eqref{eq:typeII-many-pairs}; only finitely many factors differ from $\id_2$.}\quad
\stepwhy{2}{the $\tfrac12$ cancels the $2$.}\quad
\stepwhy{3}{the ordinary partial-trace formula in each pair, with $\ket{\eta_i}=\ket{\phi_{\pi/4}}$ for the
unchanged pairs.}\quad
\stepwhy{4}{$\ket\Psi$ is a product over pairs.}
...
\end{align}
\stepwhy{1}{only the $k$ changed pairs contribute to $\log\rho_\M$. For each one, the trace factorizes pair by
...
\stepwhy{2}{$\log(2\rho)=\log2\,\id_2+\log\rho$, and the $\tfrac12$ cancels the $2$.}\quad
\stepwhy{3}{$\Tr_2\rho_{i_s}=1$, and $S_2(\rho)\equiv-\Tr_2(\rho\log\rho)$ is the ordinary single-qubit
entropy.}
...
\]
\textbf{Directly from $\rho_\M$.} Only this one factor of $\rho_\M$ differs from $\id_2$, so
\begin{align*}
...
\end{align*}
\textbf{From eq.~\eqref{eq:typeII-entropy}.} The ordinary entropy of the pair is
$S_2(\rho_{i_1})=-0.9127\log0.9127-0.0873\log0.0873\approx0.2963$, and $\log2\approx0.6931$, so
\[
S_\M(\Psi) \approx 0.2963 - 0.6931 = -0.3968 .
\]
\textbf{From ordinary quantum mechanics.} The ordinary relative entropy of the qubit state $\rho_{i_1}$ with
...
\emph{negative} number, and it equals minus an ordinary relative entropy of one qubit.
\end{workedexamplebox}
...
obeys $S_2(\rho_{i_s})-\log2\le0$. Hence $S_\M(\Psi)\le0$ for \emph{every} state built this way. Equality holds
...
contradiction, and the resolution is simple. Here $\tr$ is \emph{not} the ordinary matrix trace that counts basis
...
What eq.~\eqref{eq:typeII-entropy} actually computes is minus a \emph{relative} entropy:
\[
...
there is a minimal projection to count from. Instead, $S_\M$ is \textbf{a signed distance from the maximally
...
\end{align}
\stepwhy{1}{the definition of relative entropy, with $\sigma=\id/d$.}\quad
\stepwhy{2}{$\log(\id/d)=-\log d\,\id$ and $\Tr\rho=1$; here $S(\rho)\equiv-\Tr(\rho\log\rho)$ is the ordinary
von Neumann entropy.}
...
\subsection*{Changing infinitely many pairs: the law of large numbers}
...
\textbf{Where the state lives.} By eq.~\eqref{eq:typeII-rhoM}, $\rho_\M(\Psi_N)=(2\rho_\phi)^{\otimes
...
\end{align}
\stepwhy{1}{take the logarithm and set $m=N\cos^2\phi$.}\quad
\stepwhy{2}{$S_2(\phi)=-\cos^2\phi\log\cos^2\phi-\sin^2\phi\log\sin^2\phi$.}

\textbf{Where the trace lives.} Under the trace, each spin is up with probability $\tfrac12$, so the trace
...
\textbf{The sharp version.} This is the same law of large numbers that drives the theorem above. Apply item~3
...
\subsection*{Type $\mathrm{II}_\infty$}
...
\ \eqstep{3}\ S+\log c . \notag
\end{align}
\stepwhy{1}{substitute $\tr'=c\,\tr$ and $\rho'=\rho/c$, and use $\log(\rho/c)=\log\rho-\log c\,\id$.}\quad
\stepwhy{2}{the factors $c$ and $1/c$ cancel.}\quad
\stepwhy{3}{$\tr\rho=1$.}
...
\emph{type} of the algebra $\M$ that describes a subsystem determines, independently of the state, what
\emph{kind} of entanglement every state of the full system can have.

\begin{itemize}
\item \textbf{Type I factor.} A genuine tensor factorization $\HH=\HH_R\otimes\HH_L$ exists. Unentangled
...
\item \textbf{Type I with a center.} The Hilbert space splits into superselection sectors. The entropy is the
...
\item \textbf{Type II factor.} There are no minimal projections. A standard result says that a normal pure state
on $\M$ exists only if $\M$ has a minimal projection. So no state of the full system restricts to a pure state
on $\M$. There is no unentangled reference state anywhere. In this sense \emph{every} state of the full system
