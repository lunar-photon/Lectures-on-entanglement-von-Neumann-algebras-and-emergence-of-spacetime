\section{Sec.~II: Introduction to von Neumann algebras}
...
\emph{manufactured} from an algebra and a state, rather than assumed as a starting point. Every definition
...
\subsection{Sec.~II.A: systems with an infinite amount of entanglement (recap)}
...
section builds on it: the moral of Sec.~I was that a subsystem should be defined by \emph{what operators are
available to act with}, not by how the Hilbert space happens to split. Sec.~II.B now makes that idea into an
actual mathematical definition.

\subsection{Sec.~II.B.1: von Neumann algebras as subsystems}

\subsubsection*{Setting the stage: what a Hilbert space and an operator are, restated carefully}

Before the new definition, it's worth being completely explicit about the ingredients, because the new
definition is going to be a statement \emph{about} them, and vague pictures of ``vectors'' and ``operators''
...
just a $2\times2$ matrix, and $A\ket\xi$ is ordinary matrix-vector multiplication. The \textbf{Hermitian
...
familiar ``transpose and complex-conjugate every entry'' operation. $A$ is \textbf{self-adjoint} (or
...
$B(\HH)$ denotes the set of \emph{all} bounded operators on $\HH$ (bounded was defined in Sec.~I.B above —
informally, operators that don't stretch any vector's length by more than some fixed factor).

\subsubsection*{The new definition of subsystem}
...
\textbf{$*$-subalgebra} of $B(\HH)$ (the $*$ refers to the adjoint operation, often written $A^*$ in pure math
texts instead of $A^\dagger$).

That alone isn't quite enough. You also want $\M$ to be \emph{complete}, in the sense that if you can
...
With this notion of subsystem in hand, the \textbf{complement} of $\M$ is defined as its commutant,
\[
...
immediately: $\M'$ is \emph{not} the set of operators that all commute \emph{with one another}. It is the set
of operators that \emph{each individually} commute with \emph{every single element of $\M$}. Two different
...
Thirring's \emph{Quantum Mathematical Physics} is a standard place to look — but nothing beyond what's given
...
is that $\Delta(x-y)=0$ identically whenever $x-y$ is spacelike. The one-line reason it \emph{has} to vanish
...
companion. \textbf{So the commutant $\M(O)'$ isn't just \emph{compatible} with ``everything spacelike
separated from $O$''; in an honest relativistic field theory, everything spacelike separated from $O$ is
guaranteed, by the field theory's own equal-time commutation relations, to actually sit inside it.} What the
abstract definition of $\M'$ in this section adds on top of that familiar fact is the observation, flagged
explicitly above, that $\M(O)'$ can be \emph{strictly bigger} than $\M(O')$ — it can contain operators that
...
\subsection{Sec.~II.B.2: basic properties, \texorpdfstring{$C^*$}{C*} vs.\ von Neumann algebras, and the double commutant theorem}

\subsubsection*{Two notions of ``the limit of a sequence of operators''}
...
\textbf{Norm convergence.} Every bounded operator has a norm, $\|A\|$ — the smallest number such that
...
$\|A^\dagger A\|=1=1^2=\|A\|^2$, consistent. A sequence $A_n$ is \textbf{norm convergent} to $A$ if
$\|A_n-A\|\to0$: the maximum possible discrepancy between $A_n\ket\psi$ and $A\ket\psi$, over \emph{every}
unit vector $\ket\psi$ simultaneously, shrinks to zero. This is a strong, uniform statement.

\textbf{Weak convergence.} $A_n$ is \textbf{weakly convergent} to $A$ if $\braket{\xi|A_n|\eta}\to
\braket{\xi|A|\eta}$ for every \emph{fixed pair} of vectors $\ket\xi,\ket\eta$. This only asks that individual
...
\begin{quote}
\textit{Worked example: weak vs.\ strong vs.\ norm convergence in $\ell^2(\mathbb N)$.} Let $\HH = \ell^2(\mathbb N)$ be the infinite-dimensional Hilbert space of square-summable sequences, with standard orthonormal basis $\{\ket 1, \ket 2, \ket 3, \dots\}$. Consider the sequence of rank-1 projection operators
\[
P_n \equiv \ket n\bra n .
\]
Let us test the convergence of $P_n$ under all three topologies as $n\to\infty$:
\begin{enumerate}
\item \textbf{Weak convergence ($P_n \to 0$ weakly):} Take any two vectors $\ket\xi = \sum_{k=1}^\infty c_k\ket k$ and $\ket\eta = \sum_{k=1}^\infty d_k\ket k$ with $\sum |c_k|^2 < \infty$ and $\sum |d_k|^2 < \infty$. The matrix element is
\[
\braket{\xi|P_n|\eta} = \braket{\xi|n}\braket{n|\eta} = c_n^* d_n .
\]
By the Cauchy--Schwarz inequality for series, $\sum_{n=1}^\infty |c_n d_n| \le \sqrt{\sum |c_n|^2}\sqrt{\sum |d_n|^2} < \infty$, so the sequence of terms must converge to zero: $\lim_{n\to\infty} c_n^* d_n = 0$. Thus $\braket{\xi|P_n|\eta} \to 0$ for \emph{every} pair of states. The sequence converges weakly to the zero operator: $P_n \xrightarrow{\text{weak}} 0$.

\item \textbf{Strong convergence ($P_n \to 0$ strongly):} The strong operator topology requires $\|P_n\ket\psi\| \to 0$ for every fixed vector $\ket\psi = \sum c_k\ket k$. We compute the norm:
\[
\|P_n\ket\psi\|^2 = \|\ket n\braket{n|\psi}\|^2 = |c_n|^2 \to 0 \quad \text{as } n\to\infty .
\]
Thus $P_n \xrightarrow{\text{strong}} 0$.

\item \textbf{Norm convergence ($P_n \not\to 0$ in norm):} The operator norm measures the maximum stretch over \emph{all} normalized vectors simultaneously:
\[
\|P_n - 0\| = \sup_{\|\psi\|=1} \|P_n\ket\psi\| = \|P_n\ket n\| = \|\ket n\| = 1 .
\]
For every single $n$, $\|P_n - 0\| = 1$. It never shrinks! Hence $P_n$ does \emph{not} converge in norm to 0: $P_n \xrightarrow{\text{norm}} \text{does not exist}$.
\end{enumerate}
...
back in $\M$) is called a \textbf{$C^*$-algebra}. A $*$-subalgebra complete under the weaker, weak
convergence is called a \textbf{von Neumann algebra}. Since norm convergence automatically implies weak
...
\subsubsection*{The double commutant theorem}

Checking weak-convergence completeness directly — verifying that \emph{every} weakly convergent sequence in
...
\emph{that} ($\M''$, everything guaranteed not to disturb anything in $\M'$). The theorem says this process,
...
not yet weakly closed), $\M''$ is in general strictly bigger than $\M$ — it's the \emph{smallest} von Neumann
...
\begin{enumerate}
\item Commutator with $\sigma_z \otimes \id_L = \begin{pmatrix} \id_2 & 0 \\ 0 & -\id_2 \end{pmatrix}$:
\[
...
\item Commutator with $\sigma_x \otimes \id_L = \begin{pmatrix} 0 & \id_2 \\ \id_2 & 0 \end{pmatrix}$:
\[
...
Now compute the second commutant $\M'' = (\M')'$: an operator $Y = \begin{pmatrix} Y_{11} & Y_{12} \\ Y_{21} & Y_{22} \end{pmatrix}$ in $\M''$ must commute with every $\id_R \otimes B = \begin{pmatrix} B & 0 \\ 0 & B \end{pmatrix}$. This requires $[Y_{ij}, B] = 0$ for \emph{every} $2\times2$ matrix $B$. By Schur's lemma, the only $2\times2$ matrices commuting with all of $M_2(\mathbb C)$ are scalar multiples of the identity: $Y_{ij} = a_{ij} \id_2$. Thus
\[
...
A second example is worth doing too, because it shows what happens when $\M$ is \emph{not} simply ``all
operators on one tensor factor,'' and it previews something important for later (the notion of a
\emph{center}, coming up two subsections from now). Take the same $\HH=\mathbb C^4$, but now forget any
...
\emph{everything} in the block-diagonal algebra $\M'$, including with each other and with themselves (trivially
...
itself in this stronger sense — you're looking at an algebra with a genuinely nontrivial \emph{center}, and
...
$\Alg$ does, $\M'=\Alg'$. Given two von Neumann algebras $\M_1,\M_2$, their \textbf{join} $\M_1\vee\M_2\equiv
(\M_1\cup\M_2)''$ is the smallest von Neumann algebra containing both, and their \textbf{meet} $\M_1\wedge
...
The \textbf{center} of $\M$ is $Z(\M)\equiv\M\cap\M'$: operators that commute with everything in $\M$
\emph{and} everything in $\M'$ simultaneously — the second worked example above, with $Z(\M)=\M$ itself
(since $\M\subset\M'$ there), is exactly a case where the center is as large as it could possibly be. $\M$ is
called a \textbf{factor} (or is said to be \textbf{primary}) if its center is as small as it can possibly be
...
\subsection{Sec.~II.B.3: weights and states}

\subsubsection*{The definitions}

A \textbf{linear functional} $\omega$ on $\M$ is simply a rule assigning a complex number $\omega(A)$ to
every $A\in\M$, respecting addition and scalar multiplication: $\omega(aA+bB)=a\,\omega(A)+b\,\omega(B)$, and
compatible with the adjoint, $\omega(A^\dagger)=\omega(A)^*$ (Liu's eq.~2.17). It is a \textbf{weight} if it's
...
operator analogue of a squared magnitude) should never get assigned a negative number. It is a \textbf{state}
if, further, it's normalized, $\omega(\id)=1$ (eq.~2.21). It is a \textbf{trace} if it satisfies
...
(It \emph{can} be shown, and is stated as a fact in the paper without proof, that every physically sensible —
...
\textbf{faithful} if $\omega(A^\dagger A)=0$ forces $A=0$ (nothing nonzero is invisible to it), and it is
\textbf{pure} if it cannot be written as a nontrivial mixture $\omega=\lambda\omega_1+(1-\lambda)\omega_2$ of
two different states for some $0<\lambda<1$ (eq.~2.24) — if it \emph{can}, $\omega$ is said to \textbf{dominate}
$\omega_1$ and $\omega_2$.

\subsubsection*{What happens to the wavefunction once the algebra is taken as primary}
...
Both of those are objects that live \emph{in} or \emph{act on} a Hilbert space — you cannot even write down
...
and a state $\omega$ on it (an assignment of an expected outcome to every element of $\M$), and \emph{only
afterward}, if you want one, do you construct a Hilbert space and a vector that reproduce $\omega$ — this
...
Notice also that $\omega$ is required to be \emph{linear}: $\omega(A+B)=\omega(A)+\omega(B)$. This is already
telling you that $\omega$ describes an \emph{average} over many repetitions, not a single measurement outcome
...
measure $A$ and $B$ together unless they happen to commute. But the \emph{average}, over a large ensemble of
...
does here, and it's worth walking through, because it answers ``then what \emph{does} describe one individual
system'' directly. Slavnov starts from something more primitive than $\omega$: a functional $\varphi(\hat A)$,
defined not on the whole algebra linearly, but only on one maximal set of \emph{mutually compatible}
...
is that $\varphi$ belongs to some \emph{equivalence class} of functionals that all agree with your data on the
one compatible set of observables $\{Q\}$ you actually chose to measure. Slavnov's proposal, argued in detail
in his Sec.~3, is that \textbf{this equivalence class — not $\varphi$ itself — is what a conventional
...
\begin{itemize}
\item \textbf{Why quantum probability isn't ``we just don't know the hidden variable yet.''} The very same
...
measure. \emph{Which} ``quantum state'' your system is assigned to depends on an experimental choice made
...
\item \textbf{Why the Hilbert space shows up at all, and in what sense it's ``second stage.''} Slavnov states
this outright, and it's worth quoting the idea directly: \emph{Hilbert space vectors (wavefunctions), the
operators in the Hilbert space, and other purely mathematical concepts are not primary elements. These
concepts only appear at the second stage of the formulation.} The primary elements are the algebra of
...
\subsection{Sec.~II.B.4: \texorpdfstring{$C^*$}{C*} and von Neumann algebras generated by a single operator}
...
isomorphic to the algebra of \emph{continuous} functions on $\sigma(\mathcal O)$: every element of
...
any state or representation is chosen. What does get demoted is the eigen\emph{vector}: that object lives in
...
$\M(\mathcal O)$, in contrast, is isomorphic to the algebra of \emph{bounded} (not necessarily continuous)
...
about). On a continuum, the \textbf{indicator function} of a subset $I\subset\sigma(\mathcal O)$,
\[
...
the continuum. The operator $P_I\equiv f_I(\mathcal O)$ this corresponds to is exactly a \textbf{spectral
...
\subsection{Sec.~II.B.5: projections}

\subsubsection*{Definitions, and what they mean physically}
...
\begin{quote}
\textit{Worked example: spectral projections and Born rule on a single qubit.} Let $A = \sigma_x + \sigma_z = \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix}$ be an observable on $\mathbb C^2$. Let us construct its spectral projections explicitly:
\begin{enumerate}
\item \textbf{Eigenvalues:} The characteristic equation is $\det(A - \lambda\id) = \lambda^2 - 2 = 0$, giving eigenvalues $\lambda_\pm = \pm\sqrt{2}$.
\item \textbf{Spectral projections via Lagrange interpolation:} For any $2\times2$ matrix with distinct eigenvalues $\lambda_1, \lambda_2$, the projection onto the eigenspace of $\lambda_1$ is $P_1 = \frac{A - \lambda_2\id}{\lambda_1 - \lambda_2}$. Thus:
\[
...
\]
\item \textbf{Verification of projector axioms:}
\begin{itemize}
\item $P_+ + P_- = \frac{1}{2\sqrt2}\begin{pmatrix} 2\sqrt2 & 0 \\ 0 & 2\sqrt2 \end{pmatrix} = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix} = \id_2$.
\item $P_+ P_- = \frac{1}{8}\begin{pmatrix} 1+\sqrt2 & 1 \\ 1 & \sqrt2-1 \end{pmatrix}\begin{pmatrix} \sqrt2-1 & -1 \\ -1 & \sqrt2+1 \end{pmatrix} = \begin{pmatrix} 0 & 0 \\ 0 & 0 \end{pmatrix} = 0$.
\item $P_+^2 = P_+$ and $P_-^2 = P_-$.
\item Spectral decomposition: $\lambda_+ P_+ + \lambda_- P_- = \sqrt2 P_+ - \sqrt2 P_- = \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix} = A$.
\end{itemize}
\item \textbf{Born rule probabilities:} Suppose the system is in the state $\ket\psi = \ket0 = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$. The probability of measuring eigenvalue $+\sqrt2$ is:
\[
...
a \textbf{partial isometry} $V\in\M$ — an operator that maps its input subspace to its output subspace
...
A nonzero projection $P$ is called \textbf{finite} if it is \emph{not} equivalent (in the sense just defined)
to any strictly smaller projection $Q<P$ inside $\M$; it is called \textbf{infinite} if such a smaller-but-
equivalent $Q$ does exist. This definition takes some getting used to, because it explicitly does \emph{not}
...
Take $\M=B(\HH_1)\otimes\id_2$ on $\HH=\HH_1\otimes\HH_2$, with $\HH_2$ \emph{infinite}-dimensional, and let
...
infinite-dimensional $\HH_2$. And yet $P$ is a \emph{finite} projection of the algebra $\M$: there is no
...
\emph{abstract algebra} is isomorphic to $B(\HH_1)$ — a single copy of the algebra of all operators on
$\HH_1$, i.e., a single irreducible representation of ``what an observer with access to $\HH_1$ can do.'' The
factor $\HH_2$ in $\HH=\HH_1\otimes\HH_2$ is doing nothing but counting \emph{how many copies} of that one
irrep sit side by side inside the big Hilbert space (one copy for every basis vector of $\HH_2$). The
projection $P=\ket\psi\rangle\langle\psi|\otimes\id_2$ picks out one basis direction \emph{within a single
copy} of the irrep, while touching every one of the (infinitely many) copies simultaneously — which is
precisely why it's ``as small as $\M$ can make something'' (finite, in fact minimal, within the algebra)
while still being infinite-dimensional as a raw subspace of $\HH$. \textbf{Finiteness, in this algebra-relative
...
substance of the classification two subsections from now) an algebra can fail to have \emph{any} minimal
projections at all, while still having plenty of finite ones.

For a von Neumann \emph{factor} $\M$ specifically, the identity operator $\id$ itself is either finite or
...
in the paper), and $\M$ is called a \textbf{finite von Neumann factor}; if $\id$ is infinite, $\M$ is called an
\textbf{infinite von Neumann factor}, and in that case it turns out (also stated without proof) that any two
...
\subsection{Sec.~II.C: classification of von Neumann factors}

\subsubsection*{Building a dimension function out of nothing but $\sim$ and $\le$}
...
\emph{factor} $\M$, there exists a \textbf{dimension function} $d(P)\ge0$, defined for every projection
...
anything worthy of being called a ``size.'' This $d$ can equally be packaged as a \textbf{trace}
...
It's worth being explicit about \emph{why} the ordinary formula ``dimension $=\Tr P$'' (using the honest
...
is the concrete sense in which $\tr$ is a \emph{renormalized} trace: it strips away the (physically
...
\begin{itemize}
\item \textbf{Type I: $\M$ has minimal projections.} Normalize $d$ so that a minimal projection gets $d=1$
...
The single most important fact tying this back to ordinary quantum mechanics: it can be shown that $\M$ is a type I factor \emph{if and only if} there exists a genuine Hilbert space factorization $\HH=\HH_R\otimes\HH_L$ with
\[
...
\textbf{Theorem:} A von Neumann algebra $\M \subseteq B(\HH)$ is a type I factor if and only if there exists a unitary isomorphism $U: \HH \xrightarrow{\sim} \HH_R \otimes \HH_L$ such that:
\[
...
\item \textbf{Equivalence via partial isometries:}
...
\]
Because $P$ is minimal, $P \M P = \mathbb{C} P$. Therefore, $V_i^\dagger A V_j$ is \emph{strictly proportional to $P$}:
\[
...
\item \textbf{The commutant and trace:}
...
\item \textbf{Type II: $\M$ has finite projections but no minimal ones.} This is the genuinely new
...
there's no smallest one to stop the chain), forces $d(P)$ to take a \emph{continuous} range of values,
starting arbitrarily close to zero. This is worth pausing on: it means a von Neumann algebra can assign a
\emph{real number}, not just an integer, as the ``dimension'' of a subspace — something with no counterpart
...
\item \textbf{Type III: every nonzero projection is infinite.} The most extreme case: not even the
finite/infinite distinction has any nontrivial content, because \emph{nothing} is finite except zero itself.
...
\subsection{Sec.~II.D: ``emergent'' Hilbert space and von Neumann algebra — the GNS construction}

\subsubsection*{Why this construction is needed}
...
\emph{built}, rather than assumed, such that $\omega$ turns out to be represented by an ordinary vector in it?
...
\subsubsection*{The construction, built up in stages}

\textbf{Stage 1: treat algebra elements themselves as candidate vectors.} This is the conceptual leap, so it's
...
should count as representing the \emph{same} physical state if no measurement, using $\omega$, could ever
tell them apart.

\textbf{Stage 2: use $\omega$ to build an inner product on $\Alg$ itself.} Define, for any $A,B\in\Alg$,
\[
...
\textbf{Stage 3: handle the possibility of ``zero-length'' elements.} There might be nonzero $X\in\Alg$ with
...
\textbf{Stage 4: quotient and complete.} Define an equivalence relation $A\sim A+X$ for any $X\in\mathcal J$
...
\textbf{Stage 6: identify the special vector.} The equivalence class of the identity element, $[\id]$, is
...
same bookkeeping already used to check $\braket{0|a^n(a^\dagger)^n|0}=n!$ — only now read as \emph{defining}
...
among candidates that already satisfy $\omega$; it \emph{is} $\Alg/\mathcal J$, completed. Its points are
...
inside \emph{that particular} $\omega$'s own GNS space, never a single object shared across backgrounds. That
...
approximated by some $\pi_\omega(A)\ket\Omega$). A vector with this property is called \textbf{cyclic} — every
...
\textbf{Nothing here is new physics — this is the identical Fock-space construction from your first course
...
reference state genuinely produces an \emph{inequivalent} Hilbert space out of the very same algebra — are
...
\begin{enumerate}[leftmargin=*]
\item \textbf{System 1: Pure State on a Single Qubit ($\Alg = M_2(\mathbb{C})$)}
\begin{itemize}[leftmargin=*]
\item \emph{Abstract Algebra:} $\Alg = M_2(\mathbb{C})$, the 4-dimensional vector space spanned by transition matrices $e_{ij} \equiv \ket{i}\bra{j}$ for $i,j \in \{0,1\}$.
\item \emph{State Functional:} Let $\omega_\uparrow$ be the pure spin-up state: $\omega_\uparrow(A) \equiv \braket{0|A|0} = A_{00}$.
\item \emph{Pre-Inner Product on $\Alg$:} For any $A, B \in M_2(\mathbb{C})$:
\[
\braket{A|B}_\Alg = \omega_\uparrow(A^\dagger B) = (A^\dagger B)_{00} = A_{00}^* B_{00} + A_{10}^* B_{10} .
\]
Notice that the second column elements ($A_{01}, A_{11}, B_{01}, B_{11}$) do not appear at all!
\item \emph{Null Ideal $\mathcal{J}$:} An element $X \in \Alg$ is null if and only if:
\[
\braket{X|X}_\Alg = |X_{00}|^2 + |X_{10}|^2 = 0 \iff X_{00} = 0 \quad \text{and} \quad X_{10} = 0 .
\]
Thus the null space is the 2-dimensional left ideal:
\[
\mathcal{J} = \left\{ \begin{pmatrix} 0 & a \\ 0 & b \end{pmatrix} : a,b \in \mathbb{C} \right\} = \mathrm{span}\{e_{01}, \, e_{11}\} .
\]
\item \emph{Quotient Space $\HH_\omega = \Alg/\mathcal{J}$:} Modulo $\mathcal{J}$, every matrix collapses to its first column:
\[
[A] = A_{00} [e_{00}] + A_{10} [e_{10}] + \underbrace{A_{01}[e_{01}] + A_{11}[e_{11}]}_{= 0 \text{ in } \Alg/\mathcal{J}} = A_{00}[e_{00}] + A_{10}[e_{10}] .
\]
The quotient space is strictly \textbf{2-dimensional}! Define the orthonormal basis:
\[
\ket{0} \equiv [e_{00}], \qquad \ket{1} \equiv [e_{10}] .
\]
Computing their inner products: $\braket{0|0} = 1$, $\braket{1|1} = 1$, $\braket{0|1} = 0$. Hence $\HH_\omega \cong \mathbb{C}^2$.
\item \emph{Representation $\pi_\omega$ and Cyclic Vector:} Left multiplication by $A$ gives:
\[
\pi_\omega(A)\ket{0} = [A e_{00}] = A_{00}\ket{0} + A_{10}\ket{1}, \qquad \pi_\omega(A)\ket{1} = [A e_{10}] = A_{01}\ket{0} + A_{11}\ket{1} .
\]
This is identically ordinary matrix multiplication on $\mathbb{C}^2$!
The cyclic vector is the class of the identity:
\[
\ket\Omega = [\id_2] = [e_{00} + e_{11}] = [e_{00}] = \ket{0} .
\]
The GNS construction successfully compressed the 4D matrix algebra into the familiar 2D qubit state space $\mathbb{C}^2$.
\end{itemize}

\item \textbf{System 2: Mixed / Thermal State on $M_2(\mathbb{C})$ (Emergence of the Thermofield Double)}
\begin{itemize}[leftmargin=*]
\item \emph{State Functional:} Let $\omega_\beta(A) \equiv \Tr(\rho_\beta A)$ with full-rank density matrix $\rho_\beta = \begin{pmatrix} p_0 & 0 \\ 0 & p_1 \end{pmatrix}$, where $p_0, p_1 > 0$ and $p_0 + p_1 = 1$.
\item \emph{Pre-Inner Product on $\Alg$:}
\[
\braket{A|B}_\Alg = \Tr(\rho_\beta A^\dagger B) = p_0 (A_{00}^* B_{00} + A_{10}^* B_{10}) + p_1 (A_{01}^* B_{01} + A_{11}^* B_{11}) .
\]
\item \emph{Null Ideal $\mathcal{J}$:} Check when $\braket{X|X}_\Alg = 0$:
\[
p_0 (|X_{00}|^2 + |X_{10}|^2) + p_1 (|X_{01}|^2 + |X_{11}|^2) = 0 .
\]
Because both $p_0 > 0$ and $p_1 > 0$, every single matrix entry must vanish: $X = 0$.
Therefore, \textbf{the null ideal is trivial}: $\mathcal{J} = \{0\}$!
\item \emph{The GNS Hilbert Space:}
\[
\HH_\omega = \Alg / \{0\} \cong M_2(\mathbb{C}) \cong \mathbb{C}^2 \otimes \mathbb{C}^2 .
\]
The Hilbert space has dimension $4$, not $2$! Define the normalized basis vectors:
\[
\ket{00} \equiv \frac{1}{\sqrt{p_0}}[e_{00}], \quad \ket{10} \equiv \frac{1}{\sqrt{p_0}}[e_{10}], \quad \ket{01} \equiv \frac{1}{\sqrt{p_1}}[e_{01}], \quad \ket{11} \equiv \frac{1}{\sqrt{p_1}}[e_{11}] .
\]
\item \emph{The Emergent Thermofield Double Vector:}
Now compute the cyclic vector $\ket\Omega \equiv [\id_2] = [e_{00} + e_{11}]$:
\[
\ket\Omega = [e_{00}] + [e_{11}] = \sqrt{p_0}\ket{00} + \sqrt{p_1}\ket{11} !
\]
\textbf{The GNS reference vector is literally the entangled Thermofield Double (TFD) pure state!}
When applied to a mixed state, GNS automatically synthesizes the purifying ancilla space $\HH_L$ out of thin air, with $\pi_\omega(\Alg) = M_2(\mathbb{C}) \otimes \id$ and commutant $\pi_\omega(\Alg)' = \id \otimes M_2(\mathbb{C})$.
\end{itemize}

\item \textbf{System 3: CCR Bosonic Oscillator (Emergence of Fock Space)}
\begin{itemize}[leftmargin=*]
\item \emph{Abstract Algebra:} The $*$-algebra of polynomials in ladder operators $\{a, a^\dagger\}$ satisfying $[a, a^\dagger] = \id$.
\item \emph{State Functional:} The Gaussian vacuum state defined algebraically by $\omega_0(\id) = 1$ and $\omega_0(a^\dagger a) = 0$. Repeated use of $[a, a^\dagger] = \id$ determines all monomials: $\omega_0\big(a^m (a^\dagger)^n\big) = n!\,\delta_{mn}$.
\item \emph{Null Ideal $\mathcal{J}$:} Any element ending with an annihilation operator belongs to the null ideal: $\mathcal{J} = \Alg a$.
Modulo $\mathcal{J}$, every operator reduces to a polynomial in $a^\dagger$ alone: $[A] = \sum_n c_n [(a^\dagger)^n]$.
\item \emph{Inner Product and Orthonormal Basis:}
\[
\Braket{[(a^\dagger)^m] \Big| [(a^\dagger)^n]} = \omega_0\big(a^m (a^\dagger)^n\big) = n!\,\delta_{mn} .
\]
Normalizing these vectors yields the standard Fock basis:
\[
\ket{n} \equiv \frac{1}{\sqrt{n!}}\big[(a^\dagger)^n\big], \qquad \braket{m|n} = \delta_{mn} .
\]
\item \emph{Representation Action:}
\[
\pi_\omega(a^\dagger)\ket{n} = \sqrt{n+1}\,\ket{n+1}, \qquad \pi_\omega(a)\ket{n} = \sqrt{n}\,\ket{n-1} ,
\]
since $[a (a^\dagger)^n] = n [(a^\dagger)^{n-1}] + \underbrace{[(a^\dagger)^n a]}_{= 0 \text{ mod } \mathcal{J}} = n [(a^\dagger)^{n-1}]$.
Hilbert space completion yields the infinite-dimensional Fock space $\HH_{\text{Fock}} = \ell^2(\mathbb{N})$! $\blacksquare$
\end{itemize}
\end{enumerate}
...
\subsubsection*{Two structural facts, and what they mean}
...
\begin{itemize}
\item $\pi_\omega$ is \textbf{irreducible} (meaning $B(\HH_\omega)=\pi_\omega(\Alg)''$ — the representation
already generates \emph{every} bounded operator on $\HH_\omega$, leaving nothing outside its reach) if and
only if $\omega$ is a \textbf{pure} state.
\item $\ket\Omega$ is \textbf{separating} for $\pi_\omega(\Alg)$ (meaning $A\ket\Omega=0$ forces $A=0$ — no
nonzero operator in the algebra can annihilate the reference vector) if and only if $\omega$ is
\textbf{faithful}.
\end{itemize}

The combination \emph{cyclic and separating} is worth flagging now, even though its full importance only
...
\subsubsection*{The worked example, done completely with numbers}
...
ignores the third basis direction, so $\omega$ will turn out to be a state that is \emph{not} faithful
...
This is worth recognizing for what it is: $\ket\Omega$ is exactly the \textbf{purification} of $\rho$ you may
...
$\HH_L$ \emph{is} the completion $\Alg/\mathcal J$ from Stage 4 above, and it appears with exactly the right
...
\begin{itemize}
\item Take $\rho=\mathrm{diag}(1,0,0)$ instead — rank $1$, a pure state. Then $\HH_\omega$ collapses to a
...
\item Take $\rho=\tfrac13\id_3$ instead — full rank ($3$), maximally mixed. Then $\HH_\omega=\mathbb
C^3\otimes\mathbb C^3$, and $\ket\Omega=\tfrac1{\sqrt3}\sum_{a=1}^3\ket a_R\ket a_L$ is both cyclic
\emph{and} separating — consistent with the second Proposition, since this $\omega$ is faithful (it's the
...
\subsection{Sec.~II.E: emergent von Neumann algebras of the entangled spin example}
...
Define the algebra of \textbf{finite-energy operations}, $\Alg$: operators of the form
\[
...
produces a Hilbert space $\HH_{\Phi_\theta}$ — and this \emph{is} the space of finite-energy excitations
...
only on the $R$-spins, completed under weak convergence \emph{within this specific $\HH_{\Phi_\theta}$}, is a
von Neumann algebra — this is the object whose \emph{type} (I, II, or III, depending on $\theta$) is the
...
space $\HH_{\Phi_\theta}$ can be reconstructed using \emph{only} the $R$-algebra, with no reference to $L$ at
...
(eq.~2.58). This $\omega_\theta$ is a \emph{faithful} state of $\Alg_R$ — even though, restricted to the
full $\Alg$ from before, $\omega_\theta$ was not faithful there (a subtlety worth noting rather than glossing
over: faithfulness is a property of a state \emph{relative to} a specific algebra, not an absolute property of
...
\emph{type} — in the precise Sec.~II.C sense — is $\M_R$? Section~III of this companion answers this directly,
