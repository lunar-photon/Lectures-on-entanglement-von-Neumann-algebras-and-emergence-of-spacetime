...
The foundational engine making this modular reconstruction possible is the celebrated \textbf{JLMS theorem} (Jafferis, Lewkowycz, Maldacena, and Suh, 2015). While frequently quoted as a definition, it can be derived directly from the quantum equality of relative entropies:

\begin{keyresult}[: Derivation of the JLMS Formula and Modular Flow Equivalence]
\textbf{Theorem (JLMS):} For a boundary spatial region $A$ and its bulk entanglement wedge $b_A$ bounded by the Ryu--Takayanagi surface $\gamma_A$:
\begin{enumerate}
\item The boundary and bulk modular Hamiltonians satisfy the operator identity on the code subspace:
\[
H_A \equiv -\log\sigma_A = \frac{\hat A[\gamma_A]}{4G_N} + H_{\text{bulk}} + \mathcal{O}(G_N^{1/2}) , \qquad H_{\text{bulk}} \equiv -\log\sigma_{b_A} .
\]
\item For any bulk field operator $\Phi \in \widetilde\M_{b_A}$ in the entanglement wedge:
\[
...
\item \textbf{Relative entropy equivalence:}
...
\item \textbf{Expansion in modular Hamiltonians:}
...
\item \textbf{Ryu--Takayanagi / FLM formula:}
...
\item \textbf{Exact cancellation of bulk entropy:}
...
\item \textbf{Promotion to operator identity:}
Because this equality of expectation values holds for \emph{every} state $\rho$ in the code subspace, the operators themselves must be equal within the code subspace:
\[
...
This lets you \emph{detect} the presence of an island using only data intrinsic to the radiation
...
\textbf{The Island Rule for Radiation:} In an evaporating black hole coupled to a non-gravitational radiation reservoir, the generalized entanglement entropy of the radiation $R$ at boundary time $t$ is determined by extremizing over all possible interior quantum extremal surfaces (QES) $\partial I$:
\[
S(R) = \min_{\text{QES } I} \operatorname{ext}_I \left[ \frac{\operatorname{Area}(\partial I)}{4G_N} + S_{\text{bulk}}(R \cup I) \right] .
\]

\textbf{Explicit Derivation of the Unitary Page Curve:}
\begin{enumerate}[leftmargin=*]
...
\item \textbf{Saddle 1: The Trivial / No-Island Branch ($I = \emptyset$):}
...
\]
As $t$ increases, $S_{\text{no-island}}(R)$ grows monotonically and unboundedly, eventually exceeding the Bekenstein--Hawking entropy of the remaining black hole — this is the classic \textbf{Hawking information paradox}.

\item \textbf{Saddle 2: The Island Branch ($I \ne \emptyset$):}
...
The spatial union $R \cup I$ contains \emph{both} the outgoing Hawking quanta $c_k \in R$ \emph{and} their infalling entangled partner modes $b_k \in I$.
Within $R \cup I$, each entangled pair $\ket{\text{Hawking pair}}_k$ forms an unentangled pure state!
The infalling modes and outgoing modes \textbf{purify each other}:
\[
...
\item \textbf{The Page Time and Phase Transition:}
...
\]
Equating the two branches yields the celebrated \textbf{Page time} $t_P$:
\[
